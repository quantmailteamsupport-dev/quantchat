...
SERVER_IP = "20.249.208.224"
BASE_HTTP = f"http://{SERVER_IP}"
TIMEOUT_SECONDS = 12
...
def test_api_gateway_healthz_public_200():
    response = fetch(f"{BASE_HTTP}:4000/healthz")
    assert response.status_code == 200
    data = response.json()
...
def test_api_gateway_readyz_public_200():
    response = fetch(f"{BASE_HTTP}:4000/readyz")
    assert response.status_code == 200
    data = response.json()
